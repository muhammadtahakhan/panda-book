...
{'value':"SAT", 'viewValue':"SAMAO"}
  ...
  constructor(private spinner: NgxSpinnerService, private _snackBar: MatSnackBar, private route: ActivatedRoute,private router: Router, private globalEvents: GlobalEventService, private auth:AuthService,  private formBuilder: FormBuilder) {
    ...
    if(userId){
      ...
      this.auth.getUserById(userId).subscribe({
        next:(res:any)=>{
          ...
          // this.rForm.patchValue(this.currentUser);
          this.rForm = this.createFormGroupWithBuilder(this.formBuilder);
          this.check();
        },
 ...
 protected filterOtherOccupation() {
  ...
  createFormGroupWithBuilder(formBuilder: FormBuilder) {
    ...
    return formBuilder.group({
      ...
      id: [{value:this.currentUser?.id, disabled:false}, Validators.compose([Validators.required])],
      IDENTIFICATION_TYPE: [{value:this.currentUser?.identification_type, disabled:false}, Validators.compose([Validators.required])],
      RESIDENTIAL_STATUS: [{value:this.currentUser?.residential_status, disabled:false}, Validators.compose([Validators.required])],
      UIN: [{value:this.currentUser?.uin, disabled:false}, Validators.compose([ ])],
      NAME: [{value:this.currentUser?.name, disabled:false}, Validators.compose([ ])],
      MOBILE_NO: [{value:this.currentUser?.mobile, disabled:false}, Validators.compose([Validators.required])],
      ISSUE_DATE: [this.currentUser?.issue_date, Validators.compose([Validators.required])],
      EMAIL: [{value:this.currentUser?.email, disabled:false}, Validators.compose([Validators.required])],
      RELATIONSHIP: [{value:this.currentUser?.relationship, disabled:false}, Validators.compose([Validators.required])],
      IBAN_NO: [{value:this.currentUser?.iban, disabled:false}, Validators.compose([Validators.required])],
      relative_uin: [{value:this.currentUser?.relative_uin, disabled:false}],
      ProofofIBAN: [{value:this.currentUser?.ProofofIBAN, disabled:false}, Validators.compose([Validators.required])],
      proofofRelationship: [{value:this.currentUser?.proofofRelationship, disabled:false}, Validators.compose([Validators.required])],

      account_type: [this.currentUser?.account_type, Validators.compose([])],
      salutation: [this.currentUser?.salutation, Validators.compose([])],
      father_husband_name: [this.currentUser?.father_husband_name, Validators.compose([,  ])],
      nationality: [this.currentUser?.nationality, Validators.compose([])],
      material_status: [this.currentUser?.material_status, Validators.compose([])],
      date_of_expiry: [this.currentUser?.date_of_expiry, Validators.compose([])],
      lifetime: [this.currentUser?.lifetime, Validators.compose([  ])],
      date_of_birth: [this.currentUser?.date_of_birth, Validators.compose([])],
      mailing_address: [this.currentUser?.mailing_address, Validators.compose([])],
      country: [this.currentUser?.country, Validators.compose([])],
      province: [this.currentUser?.province, Validators.compose([])],
      city_town_village: [this.currentUser?.city_town_village, Validators.compose([])],
      other_province_state: [this.currentUser?.other_province_state],
      other_city_town_village: [this.currentUser?.other_city_town_village],
      phone_office: [this.currentUser?.phone_office, Validators.compose([  ])],
      phone_residence: [this.currentUser?.phone_residence, Validators.compose([  ])],
      fax: [this.currentUser?.fax, Validators.compose([  ])],
      permanent_address: [this.currentUser?.permanent_address, Validators.compose([  ])],
      permanent_country: [this.currentUser?.permanent_country],
      permanent_province: [this.currentUser?.permanent_province],
      permanent_city_town: [this.currentUser?.permanent_city_town],
      permanent_other_province: [this.currentUser?.permanent_other_province],
      permanent_other_city_town: [this.currentUser?.permanent_other_city_town],
      permanent_phone_office: [this.currentUser?.permanent_phone_office, Validators.compose([  ])],
      permanent_phone_residence: [this.currentUser?.permanent_phone_residence, Validators.compose([  ])],
      permanent_fax: [this.currentUser?.permanent_fax],
      gross_annual_income: [this.currentUser?.gross_annual_income, Validators.compose([])],
      source_income: [this.currentUser?.source_income, Validators.compose([, ])],
      shareholder_category: [this.currentUser?.shareholder_category, Validators.compose([ ])],
      occupation: [this.currentUser?.occupation, Validators.compose([])],
      other_occupation: [this.currentUser?.other_occupation],
      job_title: [this.currentUser?.job_title,  Validators.compose([  ])],
      department: [this.currentUser?.department,  Validators.compose([  ])],
      employer_name: [this.currentUser?.employer_name,  Validators.compose([  ])],
      employer_address: [this.currentUser?.employer_address,  Validators.compose([  ])],
      relative_name: [this.currentUser?.relative_name,  Validators.compose([  ])],
      bank_name: [this.currentUser?.bank_name, Validators.compose([ ])],
      opt_code: [this.currentUser?.opt_code],
      CNIC_FRONT: [this.currentUser?.CNIC_FRONT],
      CNIC_BACK: [this.currentUser?.CNIC_BACK],
      ADD_PROOF: [this.currentUser?.ADD_PROOF],
      SIGNATURE_PROOF: [this.currentUser?.SIGNATURE_PROOF],
      EMP_ADD_PROOF: [this.currentUser?.EMP_ADD_PROOF],
      submit_pmex_status: [this.currentUser?.submit_pmex_status],
  ...
  onSubmit(){
    ...
    ,"PERMANENT_CELL_NO":this.currentUser.mobile
    ,"PERMANENT_EMAIL":this.currentUser.email
    ...
    ,"ProofofIBAN":data.ProofofIBAN?.replace(/^data:image\/[a-z]+;base64,/, "")
    ,"proofofRelationship":data.proofofRelationship?.replace(/^data:image\/[a-z]+;base64,/, "")
    ...
    ,"relative_uin":data.relative_uin
  ...
  generateOtp(){
    ...
    data['Relative_UIN'] = data['relative_uin'];
    try {
      // data.ISSUE_DATE = this.rForm.value.ISSUE_DATE.format('DD-MMM-yyyy');
      data.ISSUE_DATE = moment(data.ISSUE_DATE).format('DD-MMM-yyyy');
    } catch (error) {
  ...
  updateUserData(submit_pmex_status:any=null){
    ...
    if(this.rForm.valid){
          ...
          //   data.ISSUE_DATE = moment(data.ISSUE_DATE).format('DD-MMM-yyyy');
          // }
          data['ProofofIBAN'] = data.ProofofIBAN ? data.ProofofIBAN?.replace(/^data:image\/[a-z]+;base64,/, ""):null;
          data['proofofRelationship'] = data.proofofRelationship ? data.proofofRelationship?.replace(/^data:image\/[a-z]+;base64,/, ""):null;
          ...
          data['identification_type']  = data['IDENTIFICATION_TYPE'];
          data['residential_status']  = data['RESIDENTIAL_STATUS'];
          data['uin']  = data['UIN'];
          data['name']  = data['NAME'];
          data['mobile']  = data['MOBILE_NO'];
          data['email']  = data['EMAIL'];
          data['relationship']  = data['RELATIONSHIP'];
          data['iban']  = data['IBAN_NO'];
          data['relative_uin']  = data['relative_uin'];
        ...
        image.onload = (rs:any) => {
          ...
          if (img_height > max_height && img_width > max_width) {
           ...
           let error =
            ...
            let basestring = e.target.result?.replace(/^data:image\/[a-z]+;base64,/, "");
            this.rForm.get(name)?.setValue(basestring);
